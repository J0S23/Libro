...
Sin valores faltantes, con 100 mediciones por tipo de presión.
...
El test de KS no rechaza la normalidad en la sistólica (p > 0.05) y sí la rechaza en la diastólica (p < 0.05), coherente con su sesgo positivo y leptocurtosis.
...
F ≈ 0.93, p > 0.05: no se rechaza la igualdad de varianzas, condición que habilita el t-test con varianza combinada.
...
t(198) ≈ 33, p < 0.001: la diferencia sistólica–diastólica (~42 mmHg) es estadísticamente significativa.
...
El z-test replica el t-test (z ≈ 32, p < 0.001), resultado esperado con n = 100 por el teorema del límite central.
...
Coincide con la salida de `z.test`.
...
d ≈ 4.7: efecto enorme, la diferencia equivale a casi cinco desviaciones estándar.
...
100 precios por tipo de cuenta, con la variable codificada como factor.
...
Se separan los precios en dos muestras independientes para las pruebas de dos grupos.
...
Shapiro-Wilk rechaza la normalidad en ambos grupos (p < 0.05), coherente con las colas pesadas observadas; orienta a usar las pruebas robustas y el t-test de Welch.
...
F ≈ 22, p < 0.001: se rechaza la igualdad de varianzas, esperable con la DS de inversión 5 veces mayor; se toma con cautela por la asunción de normalidad.
...
Bartlett también rechaza (p < 0.001); al ser sensible a la no normalidad, se contrasta con Levene y Fligner-Killeen.
...
Levene confirma la heterocedasticidad (p < 0.001) sin depender de la normalidad.
...
Fligner-Killeen, la más robusta, también rechaza (p < 0.001). Las cuatro pruebas coinciden: varianzas diferentes → usar Welch.
...
Welch rechaza Ho (t ≈ 46, p < 0.001): el precio promedio de la inversión es ~6 veces mayor, la diferencia es significativa.
...
d ≈ 6.5: efecto enorme, de varias desviaciones estándar.
...
Con n = 10, Shapiro-Wilk no rechaza la normalidad (p > 0.05).
...
t(9) ≈ -0.21, p ≈ 0.84: no hay diferencia significativa entre antes y después; la intervención no produjo un cambio medible en esta muestra.
...
Coincide con la salida de `t.test` (t ≈ -0.21, p ≈ 0.84).
...
d ≈ -0.07: efecto nulo, consistente con la no significancia.
...
El peso promedio y la mediana son ligeramente mayores en el grupo de no fumadoras, con variabilidad similar entre grupos; la diferencia aparente es de magnitud pequeña.
...
Welch da p ≈ 0.13: no se rechaza Ho, no hay diferencia significativa en el peso al nacer entre los grupos.
...
El delta de Cliff es prácticamente nulo: la diferencia entre distribuciones es de magnitud despreciable, consistente con las pruebas previas.
...
Percepciones Q1 antes y después del experimento, pareadas por participante, con las variables de control codificadas como factores.
...
Medias y medianas de Q1 muy similares entre pretest y post-test, con dispersión acotada al rango de la escala.
...
Al ser Q1 una variable ordinal, la normalidad es cuestionable de entrada; el resultado orienta al método no paramétrico pareado.
...
Tres poblaciones con faltantes no balanceados (10, 7 y 9 observaciones válidas).
...
Quedan 10 empleados, 7 agricultores y 9 no expuestos (N = 26).
...
N = 26 y k = 3, con grados de libertad k−1 = 2 y N−k = 23.
...
Medias por tratamiento: 0.578 (empleados), 0.693 (agricultores) y 0.851 (no expuestos); la separación más marcada es entre no expuestos y empleados.
...
SST ≈ 0.679 = SSA ≈ 0.354 (entre tratamientos) + SSE ≈ 0.324 (error); la variación entre grupos absorbe más de la mitad de la variación total.
...
F ≈ 12.6 > F_crit(2,23) = 3.42: al menos una media difiere de las demás.
